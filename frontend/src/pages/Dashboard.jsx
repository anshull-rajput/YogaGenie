...
export default function Dashboard(){
  ...
  useEffect(()=>{
    Promise.all([api.get('/plans'),api.get('/progress')])
      .then(([a,b])=>{setPlan(a.data.plan);setProgress(b.data.progress);})
      .catch(()=>setError('We could not load your dashboard right now. Please refresh and try again.'));
  },[]);

  const hour=new Date().getHours();
  const greeting=hour<12?'Good morning':hour<17?'Good afternoon':'Good evening';
  const todayPlan=plan?.weeklySchedule?.find(x=>x.day===new Date().toLocaleDateString('en-US',{weekday:'long'}));
